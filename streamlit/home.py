        ...
        if st.session_state.signedout :
            ...
            for res in result :
                if st.session_state["id"] == res["id"]:
                    a += 1
                    st.markdown(f"<h1 style='background-color : grey ; border-radius : 30px ; margin:10px'><i style='background-color:gray ; border-radius:10px; padding-right:50px '>{a}.</i>{res["tasks"]}</h1>" , unsafe_allow_html=True)
